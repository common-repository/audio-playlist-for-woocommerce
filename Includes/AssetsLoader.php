...
class AssetsLoader
{
	...
	public function load_assets($handler)
	{
		foreach ($this->styles as $style) {
			if(is_null($style['condition']) || call_user_func($style['condition'], $handler)) wp_enqueue_style("audio-playlist-woo/{$style['name']}", $style['path'], $style['dependencies'], AUDIOPLAYLISTWOO_VERSION);
		}

		foreach ($this->scripts as $script) {
			if(is_null($script['condition']) || call_user_func($script['condition'], $handler)) {
				wp_enqueue_script("audio-playlist-woo/{$script['name']}", $script['path'], $script['dependencies'], AUDIOPLAYLISTWOO_VERSION);

				if($script['localize']) {
					wp_localize_script("audio-playlist-woo/{$script['name']}", $script['localize']['name'], $script['localize']['args']);
				}
			}
		}
	}
